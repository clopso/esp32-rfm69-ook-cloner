...
static const ModemConfig MODEM_CONFIG_TABLE[] = {
    ...
    // works: Rb1200 Fd 5000 bw10000, DCC 400 	  { CONFIG_FSK,  0x0c, 0x80,
    // 0x02,
    // 0x8f, 0x52, 0x52, CONFIG_WHITE}, // works 10/40/80 	  { CONFIG_FSK,
    // 0x0c,
    // 0x80, 0x02, 0x8f, 0x53, 0x53, CONFIG_WHITE}, // works 10/40/40
...
uint8_t spiRead(uint8_t reg)
{
    ...
    spi_transfer(
        reg &
        ~RH_RF69_SPI_WRITE_MASK); // Send the address with the write mask off
...
uint8_t spiWrite(uint8_t reg, uint8_t val)
{
    ...
    status = spi_transfer(
        reg |
        RH_RF69_SPI_WRITE_MASK); // Send the address with the write mask on
...
uint8_t spiBurstRead(uint8_t reg, uint8_t *dest, uint8_t len)
{
    ...
    gpio_set_level(CONFIG_CSN_GPIO, LOW);
    status =
        spi_transfer(reg & ~RH_RF69_SPI_WRITE_MASK); // Send the start address
                                                     // with the write mask off
    while (len--)
        *dest++ = spi_transfer(0);
    gpio_set_level(CONFIG_CSN_GPIO, HIGH);
    return status;
...
{
    spiWrite(RH_RF69_REG_03_BITRATEMSB, (x >> 8) & 0xff);
    spiWrite(RH_RF69_REG_04_BITRATELSB, (x >> 0) & 0xff);
}

void rfm69_write_ook_peak(uint8_t x)
{
    spiWrite(RH_RF69_REG_1B_OOKPEAK, x);
}

void rfm69_write_ook_fix(uint8_t x)
{
    spiWrite(RH_RF69_REG_1D_OOKFIX, x);
}

void rfm69_write_pa_level(uint8_t x)
{
    spiWrite(RH_RF69_REG_11_PALEVEL, x);
}

uint8_t rfm69_read_lna()
{
    return spiRead(RH_RF69_REG_18_LNA);
}

void rfm69_write_lna(uint8_t x)
{
    spiWrite(RH_RF69_REG_18_LNA, x);
}

void rfm69_write_rx_bw(uint8_t x)
{
    spiWrite(RH_RF69_REG_19_RXBW, x);
}

void rfm69_write_rssi_threshold(uint8_t x)
{
	spiWrite(RH_RF69_REG_29_RSSITHRESH, x);
}
...
void readFifo()
{
    ...
    uint8_t payloadlen =
        spi_transfer(0); // First byte is payload len (counting the headers)
...
{
    uint8_t x;
    x = spiRead(RH_RF69_REG_27_IRQFLAGS1);
    return x;
}

void setRxContinuousMode()
{
    gpio_pad_select_gpio(PIN_DIO2);
    gpio_set_direction(PIN_DIO2, GPIO_MODE_INPUT);

    // mudar aqui

    spiWrite(RH_RF69_REG_02_DATAMODUL,
             RH_RF69_DATAMODUL_DATAMODE_CONT_WITHOUT_SYNC |
                 RH_RF69_DATAMODUL_MODULATIONTYPE_OOK);
    spiWrite(RH_RF69_REG_01_OPMODE,
             RH_RF69_OPMODE_SEQUENCEROFF | RH_RF69_OPMODE_MODE_RX);



    while (!(readIRQflag() & RH_RF69_IRQFLAGS1_MODEREADY))
        ;
}

void setTxContinuousMode()
{
    // RFM69_IO_DIO2_DDR |= RFM69_IO_DIO2_MASK;
    gpio_pad_select_gpio(PIN_DIO2);
    gpio_set_direction(PIN_DIO2, GPIO_MODE_OUTPUT);

    // RFM69_IO_DIO2_PORT &= ~RFM69_IO_DIO2_MASK;

    gpio_set_level(PIN_DIO2, 0);

    spiWrite(RH_RF69_REG_02_DATAMODUL, (3 << 5) | (1 << 3));
    spiWrite(RH_RF69_REG_01_OPMODE, (1 << 7) | (3 << 2));
...
void setStandbyMode()
{
    spiWrite(RH_RF69_REG_01_OPMODE, (1 << 7) | (1 << 2));
    while (!(readIRQflag() & RH_RF69_IRQFLAGS1_MODEREADY))
        ;
}
